...
export function RefundDialog({
  ...
  const [selectedItems, setSelectedItems] = useState<
    { id: number; type?: string; bundleItemIds?: number[] }[]
  >([]);
  ...
  const handleBundleItemToggle = (
    ...
    setSelectedItems((prev) => {
      ...
      if (checked) {
        ...
        bundle.bundleItemIds = bundle.bundleItemIds.filter(
          (id) => id !== bundleItemId,
        );
      }
  ...
  const checkCompatibilityWarnings = (
    ...
    if (type === "bundle" && item.bundleItems) {
      const hasFrames = item.bundleItems.some((bi) =>
        bi.name.toLowerCase().includes("frame"),
      );
      const hasBraces = item.bundleItems.some((bi) =>
        bi.name.toLowerCase().includes("brace"),
      );
    ...
    if (type !== "bundle") {
      if (item.name.toLowerCase().includes("frame")) {
        warnings.push(
          "Refunding scaffold frames may affect the stability of your remaining scaffolding",
        );
      }

      if (item.name.toLowerCase().includes("brace")) {
        warnings.push(
          "Refunding cross braces may affect the stability of your remaining scaffolding",
        );
      }
    }
    ...
    for (const selectedItem of selectedItems) {
      ...
      if (
        ...
        for (const bundleItemId of selectedItem.bundleItemIds) {
          const bundleItem = item.bundleItems.find(
            (bi) => bi.id === bundleItemId,
          );
          if (bundleItem) {
      ...
      <DialogContent className="sm:max-w-[600px]">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            ...
            <DialogDescription>
              Select scaffolding components to refund and provide refund details
            </DialogDescription>
          </DialogHeader>
          ...
          <Tabs
            ...
            className="mt-4"
            onValueChange={(value) =>
              setRefundType(value as "partial" | "full")
            }
          >
            ...
            <TabsContent value="partial" className="space-y-4 pt-4">
                              ...
                              >
                                <Checkbox
                                  ...
                                  checked={selectedItems.some(
                                    (si) =>
                                      si.id === item.id &&
                                      si.bundleItemIds?.includes(bundleItem.id),
                                  )}
                                  onCheckedChange={(checked) =>
                                ...
                                >
                                  <div>
                                    ...
                                    {bundleItem.selectedOptions &&
                                      Object.entries(bundleItem.selectedOptions)
                                        .length > 0 && (
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={paymentMethod}
                  onValueChange={(value) =>
                    setPaymentMethod(value as "original" | "cash" | "card")
                  }
                >
            ...
            <TabsContent value="full" className="space-y-4 pt-4">
              <Alert>
                ...
                <AlertDescription>
                  This will refund the entire order amount and optionally
                  restock all items.
                </AlertDescription>
              </Alert>
