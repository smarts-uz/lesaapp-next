...
export default async function POSPage() {
  ...
  if (bundleIds.length > 0) {
    ...
    const bundledProductPrices = await prisma.$queryRaw<
      Array<{
        product_id: bigint;
        meta_value: string | null;
      }>
    ...
    const bundledItemMetadata = await prisma.$queryRaw<
      Array<{
        bundled_item_id: bigint;
        meta_key: string;
        meta_value: string | null;
  ...
  const constructionProducts = Object.values(groupedProducts).map((product) => {
